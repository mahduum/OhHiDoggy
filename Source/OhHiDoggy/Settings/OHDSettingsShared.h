...
#include "OHDSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EOHDAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
UENUM(BlueprintType)
enum class EOHDGamepadSensitivity : uint8
{
...
class UOHDLocalPlayer;

/**
 * UOHDSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 *               maybe change Localsettings to OHDPlatformSettings, or DeviceSettings?  Make this one UserSettings?  TBD
 *               
...
class OHHIDOGGY_API UOHDSettingsShared : public USaveGame//TODO fill from UOHDSettingsShared
{
	GENERATED_BODY()

	
public:
	DECLARE_EVENT_OneParam(UOHDSettingsShared, FOnSettingChangedEvent, UOHDSettingsShared* Settings);
...
public:
	...
	void Initialize(UOHDLocalPlayer* LocalPlayer);

	bool IsDirty() const { return bIsDirty; }
	void ClearDirtyFlag() { bIsDirty = false; }

	void SaveSettings();
	static UOHDSettingsShared* LoadOrCreateSettings(const UOHDLocalPlayer* LocalPlayer);
...
public:
	UFUNCTION()
	EOHDAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EOHDAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EOHDAllowBackgroundAudioSetting AllowAudioInBackground = EOHDAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EOHDGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EOHDGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EOHDGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EOHDGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EOHDGamepadSensitivity GamepadLookSensitivityPreset = EOHDGamepadSensitivity::Normal;
	UPROPERTY()
	EOHDGamepadSensitivity GamepadTargetingSensitivityPreset = EOHDGamepadSensitivity::Normal;
...
private:
	...
	UPROPERTY(Transient)
	UOHDLocalPlayer* OwningPlayer = nullptr;
};
