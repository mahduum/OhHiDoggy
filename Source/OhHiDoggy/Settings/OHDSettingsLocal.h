...
USTRUCT()
struct FOhHiDoggyScalabilitySnapshot
{
	GENERATED_BODY()

	FOhHiDoggyScalabilitySnapshot();
...
private:
	...
	// Performance stats
public:
	/** Returns the display mode for the specified performance stat todo */
	//EOhHiDoggyStatDisplayMode GetPerfStatDisplayState(EOhHiDoggyDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat todo */
	//void SetPerfStatDisplayState(EOhHiDoggyDisplayablePerformanceStat Stat, EOhHiDoggyStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UOhHiDoggySettingsLocal, FPerfStatSettingsChanged);
	FPerfStatSettingsChanged& OnPerfStatDisplayStateChanged() { return PerfStatSettingsChangedEvent; }

private:
	// List of stats to display in the HUD todo
	// UPROPERTY(Config)
	// TMap<EOhHiDoggyDisplayablePerformanceStat, EOhHiDoggyStatDisplayMode> DisplayStatList;
...
private:
	...
	/* Modifies the input levels based on the active mode's overrides todo*/
	//void OverrideQualityLevelsToScalabilityMode(const FOhHiDoggyScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	//FOhHiDoggyScalabilitySnapshot DeviceDefaultScalabilitySettings;//todo
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UOhHiDoggySettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
private:
	...
	//void SetVolumeForSoundClass(FName ChannelName, float InVolume);//todo
...
private:
	//void SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume);//todo
...
public:
	...
	DECLARE_EVENT_OneParam(UOhHiDoggySettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	 ...
	 */
	//todo from UCommonLocalPlayer
	//void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UOhHiDoggyLocalPlayer* LocalPlayer);

	const TMap<FName, FKey>& GetCustomPlayerInputConfig() const { return CustomKeyboardConfig; }

private:
	void LoadUserControlBusMix();//todo
