...
#include "OHDCameraMode_ThirdPerson.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "OHDCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"

namespace OHDCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UOHDCameraMode_ThirdPerson::UOHDCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FOHDPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FOHDPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FOHDPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FOHDPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FOHDPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FOHDPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FOHDPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UOHDCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UOHDCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UOHDCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UOHDCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IOHDCameraAssistInterface* TargetControllerAssist = Cast<IOHDCameraAssistInterface>(TargetController);

	IOHDCameraAssistInterface* TargetActorAssist = Cast<IOHDCameraAssistInterface>(TargetActor);
	...
	IOHDCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IOHDCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IOHDCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IOHDCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UOHDCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IOHDCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FOHDPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(OHDCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UOHDCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UOHDCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
